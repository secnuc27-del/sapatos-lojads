...
const WHATSAPP_NUMBER = "556899408384"; // número da loja
...
let imgAtual = 0;
let paginaHistorico = []; // histórico de navegação para o swipe-back

// ==== NAVEGAÇÃO ====
function navigate(pagina, catFiltro) {
  // Registrar no histórico (não duplica a página atual)
  if (paginaAtual && paginaAtual !== pagina) {
    paginaHistorico.push(paginaAtual);
    if (paginaHistorico.length > 20) paginaHistorico.shift(); // limite de 20 entradas
  }
...
// ==== NAVEGAR PARA TRÁS (SWIPE-BACK) ====
function navigateBack() {
  if (paginaHistorico.length === 0) return;
  const anterior = paginaHistorico.pop(); // não usa navigate() para não adicionar ao histórico novamente
...
function mudarImg(idx) {
  ...
  if (mainImg) {
    ...
    setTimeout(() => {
      mainImg.src = produtoAtual.imgs[idx];
      mainImg.onload = () => {
...
function init() {
  ...
  initHeroCanvas();
  initStatCounters();
  initSwipeBack(); // gesto de deslizar para voltar no celular
...
// ==== SWIPE-TO-GO-BACK (Mobile Edge Gesture) ====
function initSwipeBack() {
  // Criar o indicador visual de arraste na borda esquerda
  ...
  const EDGE_ZONE      = 40;   // px da borda esquerda para iniciar o gesto
  const TRIGGER_DIST   = 90;   // px de arraste para confirmar a navegação
  const SHOW_THRESHOLD = 10;   // px mínimos para começar a mostrar o indicador
  ...
  document.addEventListener('touchstart', (e) => {
    ...
    // Só ativa se a página tiver histórico E o toque começar na borda esquerda
  ...
  document.addEventListener('touchmove', (e) => {
    ...
    if (dx > SHOW_THRESHOLD) {
      ...
      // Muda a cor e escala quando está perto do trigger
  ...
  document.addEventListener('touchend', (e) => {
    ...
    if (dx >= TRIGGER_DIST) {
      // Animação de confirmação e depois navega
      const pill = indicator.querySelector('.swipe-back-pill');
...
function toggleTheme() {
  ...
  // 1. Ativa as transições em todos os elementos
  document.body.classList.add("theme-transitioning");

  // 2. Força o browser a computar e aplicar o estado com transições (reflow)
  //    Sem isso, o browser pode juntar as mudanças num único paint e pular a animação
  document.body.getBoundingClientRect();

  // 3. Agora troca o tema — com as transições já ativas, o browser vai animar
  if (nextTheme === "light") {
    document.documentElement.classList.add("light");
    document.documentElement.classList.remove("dark");
  } else {
  ...
  // 4. Remove a classe de transição após a animação terminar
  setTimeout(() => {
...
function openInfoModal(tipo) {
  ...
  if (!data) return;
  document.getElementById('infoModalEyebrow').textContent = data.eyebrow;
